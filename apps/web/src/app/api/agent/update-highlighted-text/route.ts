...
export async function POST(req: NextRequest) {
  try {
    const { inputs, query, user, response_mode }: AgentRequestBody = await req.json();
    ...
    // 目标 API 地址
    const targetUrl = 'https://api.dify.ai/v1/workflows/run'
            ...
            for (const line of lines) {
              ...
              if (trimmedLine && trimmedLine.startsWith('data: ')) {
                try {
                  ...
                  // 相比 workflow 的 event 是 text_chunk，这里 event 是 message
